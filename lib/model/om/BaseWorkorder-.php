...
abstract class BaseWorkorder extends BaseObject  implements Persistent {
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->job_status_id = $rs->getInt($startcol + 22);

			$this->job_type_id = $rs->getInt($startcol + 23);

			$this->workorder_type_id = $rs->getInt($startcol + 24);

			$this->caller = $rs->getString($startcol + 25);

			$this->job_scheduled_date = $rs->getString($startcol + 26);
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[22] => $this->getJobStatusId(),
			$keys[23] => $this->getJobTypeId(),
			$keys[24] => $this->getWorkorderTypeId(),
			$keys[25] => $this->getCaller(),
			$keys[26] => $this->getJobScheduledDate(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[22], $arr)) $this->setJobStatusId($arr[$keys[22]]);
		if (array_key_exists($keys[23], $arr)) $this->setJobTypeId($arr[$keys[23]]);
		if (array_key_exists($keys[24], $arr)) $this->setWorkorderTypeId($arr[$keys[24]]);
		if (array_key_exists($keys[25], $arr)) $this->setCaller($arr[$keys[25]]);
		if (array_key_exists($keys[26], $arr)) $this->setJobScheduledDate($arr[$keys[26]]);
	}
